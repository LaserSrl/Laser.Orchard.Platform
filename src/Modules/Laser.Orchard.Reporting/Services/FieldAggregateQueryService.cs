...
namespace Laser.Orchard.Reporting.Services {
    public class FieldAggregateQueryService : IFieldAggregateQueryService
    {
        ...
        public IEnumerable<AggregationResult> RunBooleanAggregation(IHqlQuery query, AggregateMethods aggregateMethod, string fieldName, string partName) {
            string groupKey = "field.Value";
            string groupValue = string.Empty;
            switch (aggregateMethod) {
                case AggregateMethods.Count:
            ...
            var result = this.RunQuery(query, fieldName, partName, groupKey, groupValue, "IntegerFieldIndexRecords");
