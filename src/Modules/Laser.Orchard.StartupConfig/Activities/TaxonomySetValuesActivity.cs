...
namespace Laser.Orchard.StartupConfig.Activities {
    public class TaxonomySetValuesActivity : Task {
        ...
        public TaxonomySetValuesActivity(IContentManager contentManager, ITaxonomyService taxonomyService) {
            ...
            try {
               ...
               // List<Int32> termsId=termParts.Select(x=>x.Id).ToList();
                ...
                foreach (var singletaxo in taxonomieList) {
                    var thetaxonomy = _taxonomyService.GetTaxonomyByName(singletaxo.PartFieldDefinition.Settings["TaxonomyFieldSettings.Taxonomy"]);
